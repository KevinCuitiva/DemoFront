/**
 * @file src\modules\tournament\pages\ManageTournaments.tsx
...
export function ManageTournaments() {
  ...
  const validateEditForm = () => {
    ...
    if (!editForm.name.trim()) nextErrors.name = "El nombre es obligatorio";
    if (!editForm.startDate) nextErrors.startDate = "Fecha de inicio obligatoria";
    if (!editForm.endDate) nextErrors.endDate = "Fecha de fin obligatoria";
    if (editForm.startDate && editForm.endDate && editForm.endDate < editForm.startDate) {
      nextErrors.endDate = "La fecha de fin no puede ser anterior al inicio";
    }
    if ((editForm.maxTeams ?? 0) < 2) nextErrors.maxTeams = "Mínimo 2 equipos";
    if ((editForm.playersPerTeam ?? 0) < MIN_PLAYERS_PER_TEAM || (editForm.playersPerTeam ?? 0) > MAX_PLAYERS_PER_TEAM) {
      nextErrors.playersPerTeam = `Debe estar entre ${MIN_PLAYERS_PER_TEAM} y ${MAX_PLAYERS_PER_TEAM}`;
    }
    if ((editForm.costPerTeam ?? 0) < 0) nextErrors.costPerTeam = "El costo no puede ser negativo";
    if ((editForm.allowedDecanaturas ?? []).length === 0) nextErrors.allowedDecanaturas = "Selecciona al menos una decanatura";
              ...
              >
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    />
                    {editErrors.name && <p className="mt-1 text-xs" style={{ color: P.primary, fontWeight: 600 }}>{editErrors.name}</p>}
                  </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          style={{ color: P.textPrimary, fontWeight: 500 }}
                        />
                        {editErrors.startDate && <p className="mt-1 text-xs" style={{ color: P.primary, fontWeight: 600 }}>{editErrors.startDate}</p>}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          style={{ color: P.textPrimary, fontWeight: 500 }}
                        />
                        {editErrors.endDate && <p className="mt-1 text-xs" style={{ color: P.primary, fontWeight: 600 }}>{editErrors.endDate}</p>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setEditForm({ ...editForm, maxTeams: value || undefined });
                        }}
                        placeholder="Ej: 16"
                        className="w-full px-4 py-3 rounded-xl border border-black/8 text-sm"
                        style={{ color: P.textPrimary, fontWeight: 500 }}
                      />
                      {editErrors.maxTeams && <p className="mt-1 text-xs" style={{ color: P.primary, fontWeight: 600 }}>{editErrors.maxTeams}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const value = Math.max(0, parseInt(e.target.value) || 0);
                          setEditForm({ ...editForm, playersPerTeam: value || undefined });
                        }}
                        placeholder="Ej: 11"
                        className="w-full px-4 py-3 rounded-xl border border-black/8 text-sm"
                        style={{ color: P.textPrimary, fontWeight: 500 }}
                      />
                      {editErrors.playersPerTeam && <p className="mt-1 text-xs" style={{ color: P.primary, fontWeight: 600 }}>{editErrors.playersPerTeam}</p>}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const value = Math.max(0, parseInt(e.target.value) || 0);
                        setEditForm({ ...editForm, costPerTeam: value || undefined });
                      }}
                      placeholder="Ej: 50000"
                      className="w-full px-4 py-3 rounded-xl border border-black/8 text-sm"
                      style={{ color: P.textPrimary, fontWeight: 500 }}
                    />
                    {editErrors.costPerTeam && <p className="mt-1 text-xs" style={{ color: P.primary, fontWeight: 600 }}>{editErrors.costPerTeam}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {editErrors.allowedDecanaturas && <p className="mt-1 text-xs" style={{ color: P.primary, fontWeight: 600 }}>{editErrors.allowedDecanaturas}</p>}
                  </div>
                </div>
